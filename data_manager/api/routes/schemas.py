...
async def get_schema(
    database: str,
    name: str,
    version: Optional[int] = Query(None, description="Specific version (None for latest)"),
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not schema_def:
            raise HTTPException(
                status_code=404, 
                detail=f"Schema {name} not found in {database}"
            )
...
async def list_schemas(
    database: Optional[str] = Query(None, description="Database filter ('mysql', 'mongodb', or None for both)"),
    name_pattern: Optional[str] = Query(None, description="Name pattern filter"),
    status: Optional[SchemaStatus] = Query(None, description="Status filter"),
...
async def search_schemas(
    ...
    database: Optional[str] = Query(None, description="Database filter"),
) -> List[Dict[str, Any]]:
    """
