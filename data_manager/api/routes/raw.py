...
class RawQueryRequest(BaseModel):
    ...
    timeout: Optional[int] = Field(None, ge=1, le=constants.RAW_QUERY_TIMEOUT, description="Query timeout in seconds")
...
def _validate_mysql_query(query: str) -> None:
    ...
    for db in system_dbs:
        if f"FROM {db}." in query_upper or f"JOIN {db}." in query_upper:
            raise HTTPException(
                status_code=400,
                detail=f"Access to system database '{db}' not allowed"
            )
    ...
    if len(query) > 10000:  # 10KB limit
        raise HTTPException(
            status_code=400,
            detail="Query too long (max 10KB)"
        )
...
def _validate_mongodb_query(query_obj: Dict[str, Any]) -> None:
    ...
    if "drop" in str(query_obj).lower():
        raise HTTPException(
            status_code=400,
            detail="Drop operations not allowed in raw queries"
        )
    ...
    try:
        ...
        if ":" in query:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail="Invalid MongoDB query format"
                )
        else:
            raise HTTPException(
                status_code=400,
                detail="MongoDB query must be JSON format"
            )


async def _execute_mysql_query(adapter, query: str, parameters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
...
async def _execute_mongodb_query(adapter, query_obj: Dict[str, Any], parameters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
