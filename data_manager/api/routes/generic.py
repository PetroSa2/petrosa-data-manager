...
async def _validate_data_against_schema(database: str, schema_name: str, data_list: List[Dict[str, Any]]) -> None:
    """
    ...
    try:
        ...
        if not validation_response.valid:
            error_msg = f"Schema validation failed for {schema_name}: {', '.join(validation_response.errors)}"
...
class QueryRequest(BaseModel):
    ...
    sort: Optional[Dict[str, int]] = Field(None, description="Sort specification (field: 1 for asc, -1 for desc)")
    limit: Optional[int] = Field(None, ge=1, le=constants.API_MAX_PAGE_SIZE, description="Maximum records to return")
...
async def get_records(
    ...
    collection: str,
    request: Request,
    filter: Optional[str] = Query(None, description="JSON filter conditions"),
    sort: Optional[str] = Query(None, description="JSON sort specification"),
...
async def insert_records(
    ...
    schema: Optional[str] = Query(None, description="Schema name for validation"),
        ...
        for operation in request.operations:
            ...
            if op_type == "insert":
                ...
                if database == "mysql":
                    ...
                    records = await adapter.query_range(collection, datetime.min, datetime.max, None)
...
def _apply_filter(records: List[Dict[str, Any]], filter_dict: Dict[str, Any]) -> List[Dict[str, Any]]:
...
def _apply_field_selection(records: List[Dict[str, Any]], fields: List[str]) -> List[Dict[str, Any]]:
